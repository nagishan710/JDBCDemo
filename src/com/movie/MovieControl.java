...
public class MovieControl {
	...
	public static void main(String[] args) throws ShorterMovieNameException {
		try {
			...
			System.out.print("No of Screen Released ");
			String noOfScreensReleased = scanner.nextLine();
			...
			Movie movie = new Movie(movieName, releaseDate,productionCost ,noOfScreensReleased,producedBy,directedBy,true);
	...
	private static void createMoviesTable(Connection conn) throws SQLException {
		String createTableSql = "CREATE TABLE IF NOT EXISTS Movies (movieName VARCHAR(100) PRIMARY KEY,releaseDate DATE,productionCost DECIMAL(10, 2),noOfScreensReleased INT,directedBy VARCHAR(100),producedBy VARCHAR(100),status BOOLEAN)";
		Statement stmt = conn.createStatement();
		stmt.execute(createTableSql);
	}

	private static void insertMovie(Connection conn, Movie movie) throws SQLException {
		String insertSql = "INSERT INTO Movie(movieName, releaseDate, productionCost, status) " + "VALUES (?, ?, ?, ?)";
	...
	private static Movie getMovie(Connection conn, String movieName) throws SQLException {
		String selectSql = "SELECT * FROM Movie WHERE movieName = ?";
	...
	private static void deleteMovie(Connection conn, String movieName) throws SQLException {
		String deleteSql = "DELETE FROM Movie WHERE movieName = ?";
